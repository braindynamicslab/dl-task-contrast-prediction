...
if __name__ == "__main__":
    ...
    print("Number of training subjects:", len(train_dataset))
    print("Number of validation subjects:", len(val_dataset))
    ...
    if args.loss == 'rc':
        init_within_subj_margin = args.init_within_subj_margin 
        init_across_subj_margin = args.init_across_subj_margin
        if args.init_within_across_path:
            within_across = np.load(args.init_within_across_path, allow_pickle=True).item()
            init_within_subj_margin = within_across['within']
            init_across_subj_margin = within_across['across']
        print("Initial Parameters: within-subj margin %.3f - across-subj margin %.1f" % (init_within_subj_margin, init_across_subj_margin))
    ...
    for epoch in range(args.epochs):
        if args.loss == 'rc':
            within_subj_margin = np.max((init_within_subj_margin * 0.5**(epoch // args.margin_anneal_step), args.min_within_subj_margin))
            across_subj_margin = np.min((init_across_subj_margin * 2**(epoch // args.margin_anneal_step), args.max_across_subj_margin))
