...
if __name__ == "__main__":
    ...
    output_name = "%s_fold%d_of_%dfolds_feat%d_s%d_c%d_lr%s_seed%d" % (args.ver, args.fold_idx, args.folds, args.n_feat_channels, args.n_samples_per_subj, args.n_channels_per_hemi, str(args.lr), args.seed)
    ...
    splits = list(kf.split(subj_ids))
    args.fold_idx -= 1
    if args.fold_idx > args.folds or args.fold_idx < 0:
        raise('Error, wrong fold idx')
    else:
        print('Training on fold', args.fold_idx, 'of', args.folds)
        train_idx, valid_idx = splits[args.fold_idx]
        print(f'Fold: {args.fold_idx}')
        val_subj_ids = subj_ids[train_idx]
        train_subj_ids = subj_ids[valid_idx]
        ...
        for epoch in range(args.epochs):
            ...
            if mean_loss < best_loss:
                save_checkpoint(model, optimizer, scheduler, epoch, "best_loss_%d.pth" % args.fold_idx, output_dir)
                best_loss = mean_loss
            if mean_corr > best_corr:
                save_checkpoint(model, optimizer, scheduler, epoch, "best_corr_%d.pth" % args.fold_idx, output_dir)
                best_corr = mean_corr
