...
def baseline_arg_parser():
    ...
    parser.add_argument('--fold_idx',
                        type=int,
                        default=0,
                        help='Specific fold index, for parallelized training')
    ...
    parser.add_argument("--batch_size",
                        ...
                        default=2,
                        choices={2, 12},
                        help="Only batch of 2 or 12 is supported right now")
    ...
    parser.add_argument("--init_within_across_path",
                        type=str,
                        default=None,
                        help="Path to file containing the initial margins")
    ...
    parser.add_argument("--group_contrast_dir",
                        type=str,
                        default='',
                        help="Directory containing the group average task contrast files for training and validation")
    
    parser.add_argument('--contrast_id',
                        type=int,
                        default=7,
                        help="ID for the contrast being trained")
    
    parser.add_argument('--multigroup',
                        type=str,
                        default='false',
                        help="Task to exclude (training) or include (testing) for using multiple types of group contrasts. Default is false, uses contrast_id.")

    parser.add_argument('--model',
                        type=str,
                        default='BrainSErfCNN',
                        help="Model to use. Select from: BrainSurfCNN, BrainSurfAttnCNN, BrainSErfCNN")
    return parser
