...
class SqueezeExcitation(nn.Module):
    def __init__(self, in_ch, ratio, activation=nn.Tanh, scale_activation=nn.Tanh):
        super().__init__()
        """Squeeze Excitation on 1D (all mesh vertices) instead of 2D"""
        self.in_ch = in_ch
        self.sq_ch = in_ch // ratio
        self.activation = activation
        self.scale_activation = scale_activation

        self.avgpool = nn.AdaptiveAvgPool1d(1)
        self.fc1 = nn.Conv1d(self.in_ch, self.sq_ch, 1) # Basically a linear layer
        self.fc2 = nn.Conv1d(self.sq_ch, self.in_ch, 1) # Basically a linear layer
        self.activation = activation()
        self.scale_activation = scale_activation()
    ...
    def _scale(self, x):
        ...
        x = self.activation(x)
        x = self.fc2(x)
        return self.scale_activation(x)
...
class DownSE(nn.Module):
    def __init__(self, in_ch, out_ch, level, mesh_dir, bias=True, ratio=16):
        ...
        if in_ch // ratio < 1:
            ratio = in_ch // 4
            print('Ratio too small, using', ratio)
        self.se = SqueezeExcitation(in_ch, ratio=ratio)
        self.conv = ResPoolBlock(in_ch, in_ch, out_ch, level+1, True, mesh_dir)
...
class BrainSErfCNN(nn.Module):
    def __init__(self, mesh_dir, in_ch, out_ch, max_level=5, min_level=0, fdim=64, init_ratio=5):
        ...
        self.down = []
        self.up = []
        self.in_se = SqueezeExcitation(in_ch, ratio=init_ratio)  # separate ratio, plays nice with ICs
        self.in_conv = MeshConv(in_ch, fdim, self.__meshfile(max_level), stride=1)
        self.out_conv = MeshConv(fdim, out_ch, self.__meshfile(max_level), stride=1)

        """Downward path"""
        for i in range(self.levels-1):
            self.down.append(DownSE(fdim*(2**i), fdim*(2**(i+1)), max_level-i-1, mesh_dir))
        self.down.append(DownSE(fdim*(2**(self.levels-1)), fdim*(2**(self.levels-1)), min_level, mesh_dir))
